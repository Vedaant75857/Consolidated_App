...
## Delegation model-selection guardrail

Every `collaboration.spawn_agent` call for a project-local specialist must set
`agent_type` explicitly. Never rely on `task_name`, the prompt text, or a
project-local TOML file alone to select the runtime model.

Use the exact role name as both the project-local specialist identity and the
`agent_type` value:

| `agent_type` | Runtime model configured by the agent role |
| --- | --- |
| `api_contract_reviewer`, `backend_reviewer`, `frontend_reviewer`, `contract_sync_agent`, `test_engineer`, `backend_implementer`, `frontend_implementer` | `gpt-5.6-luna` |
| `frontend_architect`, `backend_architect` | `gpt-5.6-sol` |
| `product_planner`, `debug_triage_agent`, `performance_reviewer` | `gpt-5.6-terra` |

Before reporting an agent's model, verify the selected `agent_type`; do not
infer the actual runtime model from the `model = ...` line in a TOML file. If
the runtime does not expose the requested role/model, state that limitation
instead of claiming the TOML model was used.
