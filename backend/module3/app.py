...
# Stable, module-isolated session folder so uploads survive backend restarts during dev.
if (
    not (os.environ.get("MODULE3_SESSION_DB_DIR") or "").strip()
    and not (os.environ.get("SESSION_DB_DIR") or "").strip()
    and not getattr(_sys, "frozen", False)
):
    os.environ["MODULE3_SESSION_DB_DIR"] = os.path.abspath(
