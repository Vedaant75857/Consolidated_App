...
/* ─── HeaderRowEditor (self-contained) ─── */
...
function HeaderRowEditor({
  ...
  const candidateHeaders =
    selectedRow !== null && rawPreview ? rawPreview[selectedRow] : null;
  ...
  if (!rawPreview || rawPreview.length === 0) {
    return (
      <div className="px-6 pb-4 text-xs text-neutral-400 dark:text-neutral-500 italic">
        No raw data available for this table.
      </div>
    );
  }
        ...
        <div className="flex gap-2">
          {selectedRow !== null && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold rounded-lg bg-primary text-white hover:bg-primary-hover transition-colors"
            >
      ...
      {candidateHeaders && (
        <div className="bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 rounded-xl p-3 space-y-2">
          <p className="text-[10px] font-bold text-blue-700 dark:text-blue-400 uppercase">
            New headers from row {selectedRow}:
          </p>
          ...
          {candidateHeaders.some(
            (c: any) => c == null || String(c).trim() === ""
          ) && (
            <p className="text-[10px] text-amber-600 dark:text-amber-400">
              Empty cells highlighted -- type custom names above.
            </p>
          )}
        </div>
      )}
          ...
          <thead className="bg-neutral-50 dark:bg-neutral-800 sticky top-0 z-10">
            <tr>
              ...
              {Array.from({ length: maxCols }, (_, i) => (
                <th
                  key={i}
                  className="px-3 py-2 text-left font-bold text-neutral-400 dark:text-neutral-500 whitespace-nowrap border-b border-neutral-200 dark:border-neutral-700"
                >
                  Col {i}
                </th>
              ))}
                  ...
                  <button
                    ...
                    className={`w-7 h-6 rounded text-[10px] font-bold transition-colors ${
                      selectedRow === ri
                        ...
                        : "bg-neutral-100 dark:bg-neutral-700 text-neutral-500 dark:text-neutral-400 hover:bg-primary/10 hover:text-primary"
                    }`}
...
export default function DataLoading({
    ...
    async (items: DataTransferItemList) => {
      ...
      try {
        ...
        const existingPaths = new Set(
          accumulatedFilesRef.current.map((f) => f.path)
        );
  ...
  const removeFile = useCallback(
    async (path: string) => {
      accumulatedFilesRef.current = accumulatedFilesRef.current.filter(
        (f) => f.path !== path
      );
      const all = accumulatedFilesRef.current;
      if (all.length === 0) {
      ...
      setFile(zipFile);
      setFileLabel(
        `${all.length} file${all.length !== 1 ? "s" : ""} ready for upload`
      );
      setFileList([...all]);
    },
    [setFile]
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              ...
              <div
                ...
                className={`relative border-2 border-dashed rounded-2xl p-10 text-center transition-all ${
                  isDragOver
                    ? "border-primary bg-primary-50/40 dark:bg-primary/10 scale-[1.01]"
                    : file
                      ? "border-emerald-200 bg-emerald-50/30 dark:bg-emerald-950/30"
                      : "border-neutral-200 dark:border-neutral-700 hover:border-primary/50 hover:bg-neutral-50/50 dark:hover:bg-neutral-800"
                }`}
              >
                {zipping ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-xs text-neutral-400 dark:text-neutral-500">
                      Drop or browse more files to add them
                    </p>
                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={() => zipInputRef.current?.click()}
                        className="inline-flex items-center gap-1 text-xs font-medium text-neutral-500 dark:text-neutral-400 hover:text-primary transition-colors"
                      >
                      ...
                      <button
                        type="button"
                        onClick={clearFile}
                        className="inline-flex items-center gap-1 text-xs text-neutral-500 dark:text-neutral-400 hover:text-primary transition-colors"
                      >
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 text-sm border border-neutral-200 dark:border-neutral-700 rounded-xl bg-white dark:bg-neutral-900 focus:outline-none focus:ring-2 focus:ring-primary/40 focus:border-transparent placeholder:text-neutral-400 dark:placeholder:text-neutral-500 transition-shadow"
              />
              <p className="text-xs text-neutral-400 dark:text-neutral-500">
                Required for AI-powered column mapping and summaries.
              </p>
            </div>
          </div>
      ...
      {step === 2 && (
        ...
        >
          ...
          {uploadWarnings.length > 0 && (
            <div className="mx-6 mt-4 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-xl p-4 space-y-2">
              <p className="text-sm font-bold text-amber-700 dark:text-amber-400">
                {uploadWarnings.length} file
                {uploadWarnings.length !== 1 ? "s" : ""} failed to parse:
              </p>
              ...
              return (
                <div key={i}>
                  <div className="flex items-center px-6 py-4 hover:bg-neutral-50/50 dark:hover:bg-neutral-800 transition-colors">
                    ...
                    >
                      <div className="w-8 h-8 rounded-lg bg-primary-50 dark:bg-primary/10 text-primary flex items-center justify-center text-xs font-bold shrink-0">
                        {inv.table_key
                          .split(".")
                          .pop()
                          ?.substring(0, 2)
                    ...
                    <span className="text-xs font-medium text-neutral-500 dark:text-neutral-400 px-3 whitespace-nowrap">
                      {inv.rows.toLocaleString()} rows &times;{" "}
                      {inv.cols.toLocaleString()} cols
                    </span>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      {isExpanded && !isHeaderEdit && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setHeaderEditTable(inv.table_key);
                          }}
                      ...
                      {!isConfirmingDelete ? (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setConfirmDeleteKey(inv.table_key);
                          }}
                          title="Delete table"
                        ...
                        >
                          <button
                            type="button"
                            onClick={() => {
                              onDeleteTable(inv.table_key);
                              setConfirmDeleteKey(null);
                              if (expandedTable === inv.table_key)
                  ...
                  {isExpanded &&
                    ...
                    (() => {
                      const tableSelectedRows =
                        selectedRowIds[inv.table_key] || new Set();
                          ...
                          {onDeleteRows && tableSelectedRows.size > 0 && (
                            <div className="flex items-center gap-2 mb-2 px-2 py-1.5 bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 rounded-lg">
                              <span className="text-xs font-bold text-red-600 dark:text-red-400">
                                {tableSelectedRows.size} row
                                {tableSelectedRows.size !== 1 ? "s" : ""} selected
                              </span>
                              <button
                                type="button"
                                onClick={() =>
                                  handleDeleteSelectedRows(inv.table_key)
                                }
          ...
          <div className="p-6 border-t border-neutral-100 dark:border-neutral-800 flex justify-end gap-3">
            <PrimaryButton onClick={onProceedToMapping} disabled={loading}>
              {loading ? <Loader2 className="animate-spin w-4 h-4" /> : null}
